...
class LoginPage(QWidget):
    ...
    def __init__(self, api: ApiClient, parent: QWidget | None = None) -> None:
        ...
        subtitle = QLabel("RegAgent · вход через учётную запись 1С")
        ...
        self.remember_check.setStyleSheet(
            f"""
            QCheckBox {{ color: rgba(255,255,255,0.72); background: transparent; spacing: 8px; }}
            QCheckBox::indicator {{
            ...
            QCheckBox::indicator:checked {{
                background: #62E0BE; border: 1px solid #62E0BE;
        ...
        self.login_btn.setStyleSheet(
            """
            QPushButton {
                background: #ffffff; color: #0a1210; border: none;
                border-radius: 24px; padding: 0 28px;
            }
    ...
    def _submit(self) -> None:
        ...
        try:
            ...
            if exc.status_code == 503:
                info_dialog(self, "Сервис недоступен", exc.message)
            return
        finally:
            self.login_btn.setEnabled(True)
