...
def _payload(candidate: Candidate, profile: RoleProfile) -> dict[str, Any]:
    ...
    prompt = {
        "instruction": (
            "Определи, связан ли фрагмент с выбранной должностью. "
            "Не считай фрагмент относящимся к должности только из-за подразделения, CRM "
            "или общего документа. Используй только предоставленный текст. "
