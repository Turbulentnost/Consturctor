...
TESTS_USER_CLARIFY_INSTRUCTION = (
    ...
    "Задачи пользователя из erp_pm: `onec.erp_tasks_current` (открытые сейчас) и "
    "`onec.erp_tasks_period` (за период, date_from/date_to YYYY-MM-DD). "
    "Задачи подчинённых руководителя: `onec.erp_subordinate_tasks` "
    "(дерево, должность, сроки; человек из JWT). "
...
RUNTIME_NETWORK_INSTRUCTION = (
    ...
    "задачи erp_pm — `onec.erp_tasks_current` / `onec.erp_tasks_period` "
    "/ `onec.erp_subordinate_tasks` (ФИО из JWT);\n"
