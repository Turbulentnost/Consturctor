...
def build_playbook_draft_prompt(
    *,
    ...
    return (
        ...
        "В required_clarifications пиши только бизнес-параметры, которые нельзя вывести из материалов "
        "(период, объём, получатель, когда запускать, критерий успеха). "
        "Не задавай вопрос, если параметр уже понятен из контекста. "
        "Например, событийный триггер, событие вместо расписания, ручной запуск, "
        "каждые N минут, ежедневно, конкретное письмо или файл — это уже ответ про запуск. "
        "Спрашивай про запуск только если из материалов нельзя вывести ни ручной, ни событийный, "
        "ни календарный режим. Не выдумывай расписание из цели вроде «контролировать сроки» "
        "или «нарушение SLA». "
