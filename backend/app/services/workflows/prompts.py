...
- runtime — машиночитаемые правила запуска ИМЕННО ЭТОГО агента из ответов пользователя
  (сайт, ключевые слова, колонки Excel, куда сохранять файл). Не подставляй чужие словари
  и не копируй примеры из схемы, если пользователь их не давал.
- kind=site_search_excel — если агент должен искать на сайте по ключам и сохранять Excel;
  иначе runtime={} или опусти поле.
- keywords — список поисковых фраз этого агента (разбей перечисления пользователя на элементы).
...
def build_clarify_prompt(
    *,
    ...
    lines.append(
        ...
        "Обязательно перенеси ответы пользователя в runtime: site_url, keywords/keyword_text, "
        "export.columns, export.destination (например desktop), export.format=xlsx — "
        "только то, что сказал пользователь для ЭТОГО агента."
    )
