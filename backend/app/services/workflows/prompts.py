...
_DEMO_EXECUTE_DRAFT_HINT = (
    ...
    "потом create, потом снова list — notify об успехе только если проверка видит встречи. "
...
_VALIDATION_RULES = (
    ...
    "кандидат шага. Код по набору (helper) — только если ответ усечён и есть dataset_id.\n"
...
def _demo_scope_hint(draft: dict[str, Any] | None) -> str:
    if draft and (draft.get("steps") or []):
        return _DEMO_EXECUTE_DRAFT_HINT
    return _DEMO_CLARIFY_HINT + "\n"
...
def build_playbook_draft_prompt(
    *,
    ...
    """Проектирование инструкции: JSON-черновик по словарю контрактов."""
    body = _clip_demo_materials(document_text=document_text, notes=notes)
    vocab_block = (
        f"\n===== СЛОВАРЬ =====\n{vocabulary}\n===== КОНЕЦ СЛОВАРЯ =====\n" if vocabulary else ""
    )
    if interactive:
        mission = (
            "Ты проектировщик ИИ-агента. По материалам ниже собери инструкцию "
            ...
            "остальные пробелы логики закрывай так же, как раньше.\n"
        ...
        clarify = (
            ...
            "и не спрашивай тему только потому, что она типичная. "
            "Регламент ужесточай: общая фраза не считается готовым правилом. "
            "Иди по функциям и шагам по очереди, не ограничивайся двумя-тремя вопросами. "
            "После рабочих пробелов крупного блока спроси: "
            "«Я вижу этот блок так: … Надо ли дополнить этот блок дополнительной информацией?» "
            "Не спрашивай повторно то, что материалы уже дают конкретным рабочим правилом. "
            "Не подставляй очевидный дефолт вместо вопроса. "
            "Не выдумывай расписание из цели вроде «контролировать сроки» "
            "или «нарушение SLA». Один askQuestion — один пробел. "
            "Для вопроса сам придумай 2–4 варианта ответа по смыслу — "
            "не «Да/Нет», если вопрос не да/нет. "
            "JSON верни после закрытых пробелов и подтверждения блоков, не вместо вопросов.\n"
        )
    else:
        mission = (
        ...
        clarify = (
            ...
            "но без которых будущий агент будет додумывать логику. "
            "Не ограничивайся заранее заданным списком тем. "
            "Не задавай вопрос, если материалы это прямо говорят. "
            "Не подставляй очевидный дефолт вместо вопроса. "
            "Не выдумывай расписание из цели вроде «контролировать сроки» "
            "или «нарушение SLA». "
    ...
    return (
        ...
        "Служебные записки / организация совещаний — entity service_note (list/search), "
        "не document. Правила из материалов (например п. 6.4 регламента) не ищи в 1С: "
        "бери из паспорта, отдельный шаг поиска документа не нужен.\n"
        ...
        "create только в свободные слоты. Сразу после create снова list — проверить, "
        "что новые встречи видны. Затем выгрузка плана в файл (desktop · spreadsheet · export). "
        "notify — после файла и только если проверка нашла записанные встречи.\n"
        f"{'После закрытых пробелов верни один JSON-объект (можно в ```json) по схеме:' if interactive else 'Верни ТОЛЬКО один JSON-объект (можно в ```json) по схеме:'}\n"
        f"{_DRAFT_SCHEMA}\n"
        f"Название: {title_from_materials(notes=notes, document_text=document_text, document_name=document_name, fallback=title or 'агент')}\n"
        f"Источник: {document_name or 'материалы'}\n"
        f"{vocab_block}\n"
...
ARTIFACTS_INSTRUCTION = (
    ...
    "RESULT.md, playbook и любые текстовые файлы пиши на русском.\n"
...
TESTS_USER_CLARIFY_INSTRUCTION = (
    ...
    "первым делом `onec.meeting_service_notes` "
    "(серверный OData, без COM; date или date_from/date_to; сущность "
    "`Document_ТД_СлужебнаяЗаписка`; в ответе тема СЗ, тема совещания, место, участники). "
    "Запасной путь тех же СЗ: `onec.odata_catalog` + `onec.odata_get` с этой entity. "
    "Не ищи такие СЗ через `onec.search_documents` (это desktop COM). "
