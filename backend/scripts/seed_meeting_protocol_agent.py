...
2. Вызови audio.transcribe с этим file_id и names — списком ФИО из Outlook (без строк «Совещания» и почтовых ящиков). Имена передаются распознаванию, чтобы фамилии не искажались («Ильченко», не «Ивченко»). Получишь segments (start/end/text), text и duration_sec. Меток говорящих нет (diarization=none). Без успешной расшифровки протокол не составляй и report.export_document не вызывай: если audio.transcribe вернул ошибку или пустой текст — сообщи об этом человеку и остановись (в WORK_RESULT напиши «расшифровка не получена», файл не создавай). Повторный вызов по тому же файлу дожидается уже идущей расшифровки, не запускай третью.
3. Разбей расшифровку на диалоги. Реплику подписывай ФИО только если в её тексте есть обращение или самопредставление, и это ФИО есть в списке Outlook или в users.list. Иначе — «Участник N». Не назначай говорящего «по голосу» и не пересказывай: каждая реплика — как в сегменте, с таймкодом. Слово, похожее на фамилию из names, замени на точное ФИО из списка. ФИО сверяй через users.list (query — фамилия); пустой ответ Constructor не значит, что человека нет — смотри note и source=erp_pm.
...
6. Протокол в 1С. Только после явного подтверждения человеком вызови onec.meeting_protocol_write action="create" один раз на всё совещание (это документ «Протокол» ТД_Протокол, как в форме Документ.ТД_Протокол.Форма.ФормаСписка; подтверждение HITL сервер выполнит сам). Передай:
   ...
   — comment — «Сформировано ИИ-агентом по аудиозаписи <имя файла>».
...
2. audio.transcribe(file_id="f-123") → 214 сегментов, 41 мин, русский текст.
3. По обращениям и списку Outlook опознал Иванова И.П. и Петрову А.С. (сверил через users.list), третий голос — «Участник 1».
...
6. Человек подтвердил → onec.meeting_protocol_write(action="create", topic="Поставки Q3", date="2026-09-01", time_start="13:00", time_end="13:45", leader="Иванов Иван Петрович", participants=["Иванов Иван Петрович","Петрова Анна Сергеевна"], agenda=["График поставок Q3"], decisions=["Утвердить график поставок"], tasks=[{"text":"Отчёт по остаткам","executor":"Петрова Анна Сергеевна","due":"2026-09-05"}], comment="Сформировано ИИ-агентом по аудиозаписи meeting.aac") → number «ДР__062_О_427», unresolved пусто; сообщил номер человеку.
...
STEPS = [
    ...
    {"id": "s2", "title": "Транскрипция", "action": "audio.transcribe(file_id) → сегменты с таймкодами. Без расшифровки протокол и docx не создавать — сообщить человеку и остановиться."},
    {"id": "s3", "title": "Диалоги по говорящим", "action": "Спикеры по контексту, обращениям и списку Outlook; ФИО через users.list; иначе «Участник N»."},
    ...
    {"id": "s6", "title": "Протокол в 1С (ТД_Протокол)", "action": "После подтверждения человеком — onec.meeting_protocol_write action=create: topic, date, time_start/time_end, leader, participants, agenda, decisions, tasks {text, executor, due}, comment. Черновик «Подготовлен», не проводится; назвать человеку номер и unresolved."},
