...
renames rows with the legacy title «Протокол совещания по аудио») a workflow that:
  ...
  - extracts agenda, decisions and assignments,
  ...
  - drafts protocol-<date>.docx with timecodes, decisions and assignments, and notifies;
...
1. Найди аудио-вложение текущего запуска: возьми его file_id из блока вложений в промпте. Если аудио-вложения нет — попроси человека приложить аудиофайл совещания и остановись. Если в задаче переданы данные совещания из Outlook (тема, дата, участники) — запомни их: тема идёт в шапку протокола, список участников — для исполнителей поручений на шаге 3 и сопоставления говорящих на шаге 5.
2. Вызови audio.transcribe ровно один раз: file_id вложения и names — список ФИО из Outlook (без строк «Совещания» и почтовых ящиков). Имена передаются распознаванию, чтобы фамилии не искажались («Ильченко», не «Ивченко»). Инструмент записывает полную расшифровку с таймкодами во временный файл и возвращает transcript_path, segment_count и duration_sec — не полный текст. Прочитай transcript_path один раз и дальше работай с этим файлом. Повторно audio.transcribe не вызывай: второй вызов не ускоряет работу, он только возвращает тот же файл. Меток говорящих нет (diarization=none). Если инструмент вернул ошибку или segment_count=0 — протокол не составляй, в 1С не пиши, report.export_document не вызывай, напиши человеку «расшифровка не получена».
3. По прочитанной расшифровке выдели пункты повестки, решения и поручения: кто / что / срок. Бери только то, что прозвучало в записи; ничего не домысливай — если срок или исполнитель не названы, пиши «(не назван)». Исполнителя пиши точным ФИО из списка Outlook; если его там нет — users.list один раз на фамилию (query — фамилия). Пустой ответ Constructor не значит, что человека нет — смотри note и source=erp_pm.
4. Протокол в 1С. Покажи человеку повестку, решения и поручения и только после его явного подтверждения вызови onec.meeting_protocol_write action="create" один раз на всё совещание (это документ «Протокол» ТД_Протокол, как в форме Документ.ТД_Протокол.Форма.ФормаСписка; подтверждение HITL сервер выполнит сам). Передай:
   ...
   — agenda — пункты повестки из шага 3; decisions — решения из шага 3 ({text, due} если срок назван);
   — tasks — каждое поручение из шага 3: {text, executor (ФИО как в 1С), due YYYY-MM-DD}; если срок или исполнитель «(не назван)» — поле не передавай;
   ...
   Документ создаётся черновиком со статусом «Подготовлен» и не проводится — его проверит секретарь. В ответе будут number и unresolved (ФИО, не найденные в 1С): номер протокола и список несопоставленных назови человеку. Без подтверждения в 1С ничего не пиши. Если человек отказался — продолжай с шага 5 без записи в 1С.
5. Разбей прочитанную расшифровку на диалоги. Реплику подписывай ФИО только если в её тексте есть обращение или самопредставление, и это ФИО есть в списке Outlook или в users.list. Иначе — «Участник N». Не назначай говорящего «по голосу» и не пересказывай: каждая реплика — как в файле, с таймкодом. Слово, похожее на фамилию из names, замени на точное ФИО из списка. users.list вызывай не больше одного раза на каждую новую фамилию (query — фамилия), не на каждую реплику и не повторно для уже проверенного человека (исполнители с шага 3 уже проверены).
6. Сохрани протокол файлом Word: вызови report.export_document с filename="protocol-<дата>", format="docx", title="Протокол совещания". У инструмента одна таблица (table) — в table отдай поручения; вторую таблицу (участники) оформи markdown-таблицей в body раздела. Структура sections:
   — шапка (heading пустой или «Сведения»): body со строками Файл, Дата записи, Длительность, Источник, Тема (тему бери из Outlook, если передана, иначе из записи), Протокол в 1С (номер с шага 4, если создан);
...
3. Повестка: график поставок Q3. Решения: утвердить график поставок. Поручения: Петрова А.С. — отчёт по остаткам до 05.09 (прозвучало в записи; ФИО из списка Outlook).
4. Показал повестку, решения и поручения; человек подтвердил → onec.meeting_protocol_write(action="create", topic="Поставки Q3", date="2026-09-01", time_start="13:00", time_end="13:45", leader="Иванов Иван Петрович", participants=["Иванов Иван Петрович","Петрова Анна Сергеевна"], agenda=["График поставок Q3"], decisions=["Утвердить график поставок"], tasks=[{"text":"Отчёт по остаткам","executor":"Петрова Анна Сергеевна","due":"2026-09-05"}], comment="Сформировано ИИ-агентом по аудиозаписи meeting.aac\\noutlook:AAMkAGI2…") → number «ДР__062_О_427», unresolved пусто; сообщил номер человеку.
5. По обращениям и списку Outlook опознал Иванова И.П. и Петрову А.С. (users.list по одному разу на фамилию), третий голос — «Участник 1».
6. report.export_document(filename="protocol-2026-09-01", format="docx", title="Протокол совещания", sections: шапка с номером ДР__062_О_427, Участники (md-таблица), Повестка, Диалоги с [мм:сс], Решения, Поручения; table=поручения Кто/Что/Срок) — файл .docx сохранён, показал человеку.
...
STEPS = [
    ...
    {"id": "s3", "title": "Повестка, решения и поручения", "action": "Кто / что / срок — только прозвучавшее; пустые поля — «(не назван)». Исполнители — ФИО из Outlook или users.list один раз на фамилию."},
    {"id": "s4", "title": "Протокол в 1С (ТД_Протокол)", "action": "После подтверждения человеком — onec.meeting_protocol_write action=create: topic, date, time_start/time_end, leader, participants, agenda, decisions, tasks {text, executor, due}, comment (+ строка outlook:<id> из задания). Черновик «Подготовлен», не проводится; назвать человеку номер и unresolved."},
    {"id": "s5", "title": "Диалоги по говорящим", "action": "Спикеры по обращениям и списку Outlook; users.list один раз на фамилию; иначе «Участник N»."},
    {"id": "s6", "title": "Протокол protocol-<дата>.docx", "action": "report.export_document format=docx, filename=protocol-<дата>, title=Протокол совещания: шапка (+ номер протокола в 1С), Участники (md-таблица в body), Повестка, Диалоги [мм:сс] (все реплики, без сокращений), Решения; table=Поручения Кто/Что/Срок."},
