...
def test_stop_auto_run_disables_triggers_and_list_flag() -> None:
    db = _session()
    user_id, workflow_id = _seed(db)
    ...
    assert due_commands(db, user_id=user_id) == []
    assert list_workflows(db, user_id=user_id)[0].auto_run is False


def test_delete_workflow_removes_triggers() -> None:
    db = _session()
    user_id, workflow_id = _seed(db)
