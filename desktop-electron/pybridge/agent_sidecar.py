...
Твоя задача — выжать из пользователя всю операционную логику регламента, прежде чем появятся ИИ-агенты.
Регламент это черновик, не готовая инструкция. Наличие текста блока не значит, что блок закрыт.
Сначала прочитай materials/regulation.md, materials/functions.md, materials/answers.md и materials/manifest.json.

Иди строго по каждому функциональному блоку из materials/functions.md, по очереди.
Не перескакивай. Не закрывай несколько блоков одним вопросом.

По каждому блоку обязательно проверь слоты. Спроси, если в тексте нет конкретного рабочего правила:
кто запускает и от чьего имени; стартовое событие; входы и откуда брать данные;
фильтр и объём выборки; условия и ветвления; конкретное действие в системе;
результат и формат; получатель; контроль, срок, эскалация; пустой ответ и ошибка;
что агент делает сам, а что только предлагает человеку.

Общая фраза регламента («контролировать», «информировать», «при необходимости»,
«в установленном порядке», «по согласованию») — это не закрытый слот, а обязательный вопрос.
Не подставляй дефолт. Не пиши «из регламента и так ясно», если правило нельзя исполнить без догадки.

После того как по блоку закрыты рабочие пробелы, один раз спроси подтверждение понимания.
Формулировка близко к такой:
«Я вижу блок «…» так: [2–4 предложения своими словами]. Надо ли дополнить этот блок дополнительной информацией?»
Варианты в options:
- достаточно, идём дальше
- дополнить — напишу что
- часть прочитать иначе — уточню
- этот блок не нужен агенту
Если пользователь дополняет или поправляет — задай следующий конкретный вопрос, затем снова подтверди блок.
К следующему блоку переходи только после подтверждения.
...
- не спрашивай повторно то, что пользователь уже ответил в materials/answers.md;
- после ответа продолжай с учётом этого ответа, не начинай заново;
- если пользователь упомянул прикреплённые файлы, считай их обязательными материалами;
- не ограничивай себя двумя-тремя вопросами: вопросов столько, сколько блоков и дыр в них;
- не пиши JSON и не говори «уточнения не нужны», пока каждый блок не пройден и не подтверждён.

Когда все блоки подтверждены, напиши строго JSON без markdown:
{
  "status": "ready",
  "blocks": [
...
def _build_readiness_prompt() -> str:
    return (
        ...
        "Иди по каждому функциональному блоку по очереди и докопайся до пользователя: "
        "закрой через askQuestion все рабочие пробелы блока "
        "(кто, данные, фильтр, условие, действие, результат, получатель, ошибка, эскалация). "
        "Общую фразу регламента не считай достаточным правилом. "
        "После пробелов блока спроси: как ты видишь этот блок и надо ли дополнить его "
        "дополнительной информацией. "
        "В каждом askQuestion передай 2-6 конкретных вариантов в options. "
        "JSON readiness верни только когда каждый блок подтверждён, не раньше."
    )
...
class Sidecar:
    ...
    @staticmethod
    def _emit_adopt_run(incoming_run_id: str, existing_run_id: str) -> None:
        """Tell the UI to listen to the already-running sidecar job.

        A second start (StrictMode remount, retry, or a stale restart) gets a
        new command id. Events from the first job still use the old id, so the
        new listener would stay on «Агент запускается…» forever unless we
        remap it.
        """
        payload = {
            "type": "adopt",
            "activeRunId": existing_run_id,
            "text": "Продолжаю текущий запуск агента.",
        }
        seen: set[str] = set()
        for rid in (incoming_run_id, existing_run_id):
            if not rid or rid in seen:
                continue
            seen.add(rid)
            emit({"type": "event", "runId": rid, "payload": payload})
    ...
    def start(self, kind: str, command: dict[str, Any]) -> None:
        ...
        if overlap_run_id:
            ...
            self._emit_adopt_run(run_id, overlap_run_id)
            return
        if skip_run_id:
            self._emit_adopt_run(run_id, skip_run_id)
            return
        emit(
            _stamp_run_event(
                {
