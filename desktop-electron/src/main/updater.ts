...
const INSTALLER_NAME = 'constructor-setup.exe'
...
function pickInstaller(assets: GithubAsset[]): GithubAsset | null {
  const exes = assets.filter((asset) => {
    const name = String(asset.name || '').toLowerCase()
    return name.endsWith('.exe') && !name.includes('blockmap')
  })
  return exes.find((asset) => asset.name.toLowerCase() === INSTALLER_NAME) || exes[0] || null
}
...
async function checkForUpdate(): Promise<void> {
  ...
  try {
    ...
    if (!release || !asset || !version || compareVersions(version, status.currentVersion) <= 0) {
      found = null
...
export async function installAvailableUpdate(): Promise<{ ok: boolean; error?: string }> {
  ...
  const dest = join(updateDir(), 'Constructor-Setup.exe')
  try {
    await downloadInstaller(target.asset, dest)
