...
const AUTONOMY = {
  ...
  forbidden:
    'Писать в календарь без подтверждения. Готовить письменный план дня, справку или комплект материалов. Делать ручную рассылку приглашений. Повторять поиск писем. Спрашивать «что должен делать агент» на рабочем запуске.'
}
...
> = {
  ...
  receives:
    ...
    checks:
    'Календарь руководителя в Outlook: утром только сегодня, вечером завтра; один поиск отсутствий; окна 09:00–18:00 кроме обеда 12:00–13:00; хвост после последней встречи окном не считать; пересечения слотов; выходные и праздники РФ.',
  decisions:
    'До 16:00 — устный список. При пересечении или окне — предложить какую встречу куда перенести (разовую сдвигать, регулярную оставлять, длительность та же, ближайшее окно 09:00–18:00 кроме обеда) и вызвать outlook.create_event: карточка HITL спрашивает разрешение. Без подтверждения не писать. search_mail не повторять, не спрашивать «что должен делать агент» и не дублировать разрешение через askQuestion. После 16:00 — то же на завтра. Если названа незапланированная встреча — тоже create_event после HITL. Если участник отсутствует — учесть.',
  canAutonomous: AUTONOMY.canAutonomous,
  needsHumanApproval: AUTONOMY.needsHumanApproval,
  forbidden: AUTONOMY.forbidden,
    result:
    'Утром — устный список и при конфликте предложение сдвига; запись в Outlook только после разрешения. Вечером — то же на завтра. Файла плана дня нет.'
}
