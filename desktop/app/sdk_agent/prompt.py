...
Если агент собирается по регламенту — ужесточи опрос: регламент это черновик. Общая фраза («контролировать», «информировать», «при необходимости») не закрывает слот. Иди по шагам и функциям по очереди, не останавливайся на двух-трёх вопросах.
По крупному шагу или функции после рабочих пробелов спроси подтверждение: «Я вижу этот блок так: … Надо ли дополнить этот блок дополнительной информацией?» Варианты: достаточно / дополнить — напишу что / прочитать иначе. К следующему шагу переходи после ответа.
...
Не спрашивай повторно то, что пользователь уже ответил конкретным рабочим правилом. Общую формулировку регламента не считай таким ответом. Не подставляй дефолт вместо вопроса.
...
def build_design_sdk_prompt(workflow: WorkflowRecord, design_prompt: str) -> str:
    ...
    return (
        ...
        "По регламенту докопайся до каждого шага и после крупного блока спроси, "
        "верно ли ты его видишь и надо ли дополнить. "
        "Когда пробелы закрыты и понимание блоков подтверждено, напиши JSON-черновик и остановись. "
