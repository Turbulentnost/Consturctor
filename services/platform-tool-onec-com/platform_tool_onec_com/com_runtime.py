"""Run COM calls on a dedicated STA thread."""
...
def _ensure_thread() -> None:
    ...
    if _thread is not None and _thread.is_alive():
        return
    _ready.clear()
    _thread = threading.Thread(target=_com_worker, name="onec-com-sta", daemon=True)
    _thread.start()
    if not _ready.wait(timeout=10.0):
        raise RuntimeError("COM STA thread failed to initialize")
